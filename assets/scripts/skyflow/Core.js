(function(W) {
    ...
    let uniqueIdName = "data-sk-id", locale = "en";

    let elementsStorage = {},
        eventsStorage = {},
        servicesStorage = {},
        singletonServicesStorage = {};
    ...
    Object.prototype["isCallback"] = function () {
        return isCallback(this);
    };
    ...
    function skyflowEventCallback(e) {
        ...
        if (!callbacks || !(callbacks = callbacks[type])) {return false}
        ...
        callbacks.forEach((callback) => {
            callback.apply(callbackObject, [e, target, type])
        });
    ...
    function triggerEvent(element, event) {
        ...
        if (!eventsStorage[uniqueId]) {return false}
    ...
    const METHODS = {
         ...
         *
         * @method on
         * @param {string} event It possible to add several events once.
         * @param {Function} callback
         * @returns {string}
        ...
        on(event, callback) {
            ...
            if (!isString(event) || !isCallback(callback)) {
                return this
            }

            let elements = CACHE.get(this);

            elements.forEach((element) => addEvent(element, event, callback));
        ...
        off(event, callback) {
            ...
            elements.forEach((element) => removeEvent(element, event, callback));
        ...
        trigger(event) {
            ...
            elements.forEach((element) => triggerEvent(element, event));
    ...
    function Helper() {
         ...
         * @class Helper
         * @example helper.add.example.js
         */
        this.add = function (helper) {

            if(!isFunction(helper)){return false}

            let name = helper.name;

            if(helpers.hasOwnProperty(name)){return false}
         ...
         * @method get
         * @param {string} name
         * @returns {Function|null} Return null if helper does not exist
         * @class Helper
         */
        this.get = function (name) {
            return helpers.hasOwnProperty(name) ? helpers[name] : null
        };
        ...
        /**
         * Checks whether a helper is defined or not.
         *
         * @method has
         * @param {string} name
         * @returns {boolean}
    ...
    function Skyflow() {
         ...
         * @returns {boolean}
         * @class Skyflow
         * @example skyflow.register.example.js
         */
        this.register = (service, singleton = true) => {

            if(!isFunction(service)){return false}

            let name = service.name;

            if(isEmpty(name)){return false}

            if(servicesStorage.hasOwnProperty(name) || singletonServicesStorage.hasOwnProperty(name)){
                return false
            }

            if(!isBoolean(singleton)){singleton = true}

            if(singleton){
                singletonServicesStorage[name] = new service();
            }else {
                servicesStorage[name] = service;
            }
         ...
         * @param {string} name
         * @param args
         * @returns {*} An exception is thrown if the service does not exist
         * @class Skyflow
         * @example skyflow.get.example.js
         */
        this.get = (name, ...args) => {
            ...
            if(singletonServicesStorage.hasOwnProperty(name)){
                return singletonServicesStorage[name]
            }

            if(servicesStorage.hasOwnProperty(name)){
                return new servicesStorage[name](...args)
            }
         ...
         */
        this.has = (name) => {
            return servicesStorage.hasOwnProperty(name) || singletonServicesStorage.hasOwnProperty(name)
        };
         ...
         */
        this.locale = (value) => {
            if(value === undefined){ return locale }
            locale = value;
